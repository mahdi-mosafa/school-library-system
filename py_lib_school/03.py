...
def lend_book(df):
    ...
    # ثبت امانت کتاب بدون بررسی وضعیت
    book_entry = df[df['Book_ID'] == book_id]
    if not book_entry.empty:
        ...
        new_entry = {
            ...
            'Book_ID': book_id,
            'Book_Title': book_entry['Book_Title'].values[0],
            'Author': book_entry['Author'].values[0],
            'ISBN': book_entry['ISBN'].values[0],
            'Operation': operation,
            'Date': date
        }
